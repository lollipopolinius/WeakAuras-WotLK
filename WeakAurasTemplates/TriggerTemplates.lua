...
-- local C _ Talent = _G.C _ Talent
...
function WeakAuras.CreateTemplateView(Private, frame)
  ...
  createButtons = function(selectedItem) -- selectedItem is either a regionType or a trigger section
    ...
    if (not newView.data) then
      ...
      if WeakAuras.regionOptions then
        for regionType, regionData in pairs(WeakAuras.regionOptions) do
          if (regionData.templates) then
    ...
    elseif (newView.data and not newView.chosenItem) then
      -- Second step: Trigger selection screen
		...
		-- Race
			-- Human = {},NightElf = {},Dwarf = {},Gnome = {},Draenei = {},Orc = {},Scourge = {},Tauren = {},Troll = {},BloodElf = {},
						
		local raceHeader = AceGUI:Create("Heading");
		raceHeader:SetFullWidth(true);
		newViewScroll:AddChild(raceHeader);

		-- local raceSelector = createDropdown("race", TemplatePrivate.triggerTemplates.race); --WeakAuras.race_types);
  ...
  function newView.Open(self, data, targetId)
    ...
    newView.spec = GetActiveTalentGroup("player") -- C _ Talent.GetSpecInfoCache().activeTalentGroup   -- возвращает активную ветку талантов, при формировании результата ветка с наибольшим очков талантов ставится на первое место
